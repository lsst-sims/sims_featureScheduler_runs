...
def generate_blobs(nside, mixed_pairs=False, nexp=1, no_pairs=False, nshort=2, short_time=1.):
    target_map = standard_goals(nside=nside)
    norm_factor = calc_norm_factor(target_map)
    ...
    for filtername, filtername2 in zip(filter1s, filter2s):
        ...
        deets.append(detailers.Short_expt_detailer(filtername=filtername, nside=nside,
                                                   nobs=nshort, exp_time=short_time,
                                                   footprint=short_footprints[filtername]))
        if (filtername2 is not None) & (filtername2 != filtername):
            deets.append(detailers.Short_expt_detailer(filtername=filtername2, nside=nside,
                                                       nobs=nshort, exp_time=short_time,
...
if __name__ == "__main__":
    ...
    if Pairs:
        if mixedPairs:
            ...
            blobs = generate_blobs(nside, nexp=nexp, mixed_pairs=True, nshort=nshort, short_time=short_time)
            surveys = [ddfs, blobs, greedy]
            run_sched(surveys, survey_length=survey_length, verbose=verbose,
                      fileroot=os.path.join(outDir, 'shortt_%ins_%iext_pairsmix_' % (nshort, short_time)), extra_info=extra_info)
        else:
            ...
            surveys = [ddfs, blobs, greedy]
            run_sched(surveys, survey_length=survey_length, verbose=verbose,
                      fileroot=os.path.join(outDir, 'shortt_%ins_%iext_pairsame_' % (nshort, short_time)), extra_info=extra_info)
    else:
        greedy = gen_greedy_surveys(nside, nexp=nexp)
        ddfs = generate_dd_surveys(nside=nside, nexp=nexp)
        blobs = generate_blobs(nside, nexp=nexp, no_pairs=True, nshort=nshort, short_time=short_time)
        surveys = [ddfs, blobs, greedy]
        run_sched(surveys, survey_length=survey_length, verbose=verbose,
                  fileroot=os.path.join(outDir, 'shortt_%ins_%iext_nopairs_' % (nshort, short_time)), extra_info=extra_info)
