    ...
    if Pairs:
        if mixedPairs:
            ...
            run_sched(surveys, survey_length=survey_length, verbose=verbose,
                      fileroot=os.path.join(outDir, 'noddf_%iexp_pairsmix_' % nexp), extra_info=extra_info)
        else:
            ...
            surveys = [blobs, greedy]
            run_sched(surveys, survey_length=survey_length, verbose=verbose,
                      fileroot=os.path.join(outDir, 'noddf_%iexp_pairsame_' % nexp), extra_info=extra_info)
    else:
        greedy = gen_greedy_surveys(nside, nexp=nexp)
        blobs = generate_blobs(nside, nexp=nexp, no_pairs=True)
        surveys = [blobs, greedy]
        run_sched(surveys, survey_length=survey_length, verbose=verbose,
                  fileroot=os.path.join(outDir, 'noddf_%iexp_nopairs_' % nexp), extra_info=extra_info)
