...
def gen_greedy_surveys(nside, nexp=1, m5w=3., footprintw=0.3, sleww=3., fcw=3.):
    """
...
def generate_blobs(nside, mixed_pairs=False, nexp=1, no_pairs=False,
                   m5w=6., footprintw=0.6, sleww=3., fcw=3.):
    target_map = standard_goals(nside=nside)
    norm_factor = calc_norm_factor(target_map)
    ...
    for filtername, filtername2 in zip(filter1s, filter2s):
        ...
        weights = np.array([m5w/2., m5w/2., footprintw/2., footprintw/2., sleww, fcw, 0., 0., 0., 0., 0., 0.])
        if filtername2 is None:
            # Need to scale weights up so filter balancing still works properly.
            weights = np.array([m5w, footprintw, sleww, fcw, 0., 0., 0., 0., 0., 0.])
        if filtername2 is None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--m5w", type=float, default=6.)
    parser.add_argument("--footprintw", type=float, default=0.6)
    parser.add_argument("--sleww", type=float, default=3.)
    parser.add_argument("--fcw", type=float, default=3.)
    ...
    tag = '%.1f_%.1f_%.1f_%.1f_' % (m5w, footprintw, sleww, fcw)

    if Pairs:
        if mixedPairs:
            # mixed pairs.
            greedy = gen_greedy_surveys(nside, nexp=nexp, m5w=m5w, footprintw=footprintw, sleww=sleww, fcw=fcw)
            ddfs = generate_dd_surveys(nside=nside, nexp=nexp)
            blobs = generate_blobs(nside, nexp=nexp, mixed_pairs=True,
                                   m5w=m5w, footprintw=footprintw, sleww=sleww, fcw=fcw)
            surveys = [ddfs, blobs, greedy]
            run_sched(surveys, survey_length=survey_length, verbose=verbose,
                      fileroot=os.path.join(outDir, 'vw_'+tag), extra_info=extra_info)
        else:
            # Same filter for pairs
            greedy = gen_greedy_surveys(nside, nexp=nexp, m5w=m5w, footprintw=footprintw, sleww=sleww, fcw=fcw)
            ddfs = generate_dd_surveys(nside=nside, nexp=nexp)
            blobs = generate_blobs(nside, nexp=nexp, m5w=m5w, footprintw=footprintw, sleww=sleww, fcw=fcw)
            surveys = [ddfs, blobs, greedy]
            run_sched(surveys, survey_length=survey_length, verbose=verbose,
                      fileroot=os.path.join(outDir, 'vw_'+tag), extra_info=extra_info)
    else:
        greedy = gen_greedy_surveys(nside, nexp=nexp, m5w=m5w, footprintw=footprintw, sleww=sleww, fcw=fcw)
        ddfs = generate_dd_surveys(nside=nside, nexp=nexp)
        blobs = generate_blobs(nside, nexp=nexp, no_pairs=True, m5w=m5w, footprintw=footprintw, sleww=sleww, fcw=fcw)
        surveys = [ddfs, blobs, greedy]
        run_sched(surveys, survey_length=survey_length, verbose=verbose,
                  fileroot=os.path.join(outDir, 'vw_'+tag), extra_info=extra_info)
