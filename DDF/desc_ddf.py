...
if __name__ == "__main__":
    ...
    parser.add_argument("--perObs", dest='perNight', action='store_false')
    parser.set_defaults(perNight=True)
    parser.add_argument("--maxDither", type=float, default=0.70)
    ...
    desc_ddfs = generate_desc_dd_surveys(nside=nside, nexp=nexp, detailers=detailers)
    ...
    if Pairs:
        if mixedPairs:
            ...
            run_sched(surveys, survey_length=survey_length, verbose=verbose,
                      fileroot=os.path.join(outDir, name+'%iexp_pairsmix_' % nexp), extra_info=extra_info)
        else:
            ...
            surveys = [ddfs, blobs, greedy]
            run_sched(surveys, survey_length=survey_length, verbose=verbose,
                      fileroot=os.path.join(outDir, name+'%iexp_pairsame_' % nexp), extra_info=extra_info)
    else:
        greedy = gen_greedy_surveys(nside, nexp=nexp)
        blobs = generate_blobs(nside, nexp=nexp, no_pairs=True)
        surveys = [ddfs, blobs, greedy]
        run_sched(surveys, survey_length=survey_length, verbose=verbose,
                  fileroot=os.path.join(outDir, name+'%iexp_nopairs_' % nexp), extra_info=extra_info)
