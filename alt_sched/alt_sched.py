...
def gen_greedy_surveys(nside, nexp=1):
    ...
    for filtername in filters:
        ...
        bfs.append(bf.Az_modulo_basis_function())
    ...
    if Pairs:
        if mixedPairs:
            ...
            run_sched(surveys, survey_length=survey_length, verbose=verbose,
                      fileroot=os.path.join(outDir, 'alt_%iexp_pairsmix_' % nexp), extra_info=extra_info)
        else:
            ...
            surveys = [ddfs, blobs, greedy]
            run_sched(surveys, survey_length=survey_length, verbose=verbose,
                      fileroot=os.path.join(outDir, 'alt_%iexp_pairsame_' % nexp), extra_info=extra_info)
    else:
        greedy = gen_greedy_surveys(nside, nexp=nexp)
        ddfs = generate_dd_surveys(nside=nside, nexp=nexp)
        ...
        surveys = [ddfs, blobs, greedy]
        run_sched(surveys, survey_length=survey_length, verbose=verbose,
                  fileroot=os.path.join(outDir, 'alt_%iexp_nopairs_' % nexp), extra_info=extra_info)
