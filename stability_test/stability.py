...
def run_sched(surveys, survey_length=365.25, nside=32, fileroot='baseline_', verbose=False,
              extra_info=None, mjd_start=59853.5, seed=42):
    years = np.round(survey_length/365.25)
    scheduler = Core_scheduler(surveys, nside=nside)
    n_visit_limit = None
    observatory = Model_observatory(nside=nside, mjd_start=mjd_start, seed=seed)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--day_offset", type=float, default=0)
    ...
    if Pairs:
        if mixedPairs:
            ...
            run_sched(surveys, survey_length=survey_length, verbose=verbose,
                      fileroot=os.path.join(outDir, 'stability_%ioffset_%iseed' % (day_offset, seed)), extra_info=extra_info,
                      mjd_start=mjd_start, seed=seed)
        else:
